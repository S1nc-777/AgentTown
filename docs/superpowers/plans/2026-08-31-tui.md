...
```ts
/**
 * TUI-executable command subset. Kept in sync with the CLI COMMANDS set in
 * ../main.js; commands outside this set (approve/reject/stop/cleanup) are
 * shown but not executed inside the TUI (see the spec).
 */
...
describe("wizardSubmit", () => {
  ...
  it("accepts employee ids and empty assignee", () => {
    let view = startWizard(CANDIDATES);
    view = step(view, "标题");
    view = step(view, "目标");
...
- Modify: `packages/cli/src/main.ts`（export `employeeStatus`、`record`、`requiredString`、`requiredNonnegativeInteger`、`friendlyError`）
- Test: `packages/cli/test/tui.test.ts`

**Interfaces:**
- Consumes: `KeyParser`/`applyEditorKey`/`createEditor`/`rememberHistory` from `./input.js`；`renderFrame`/`TuiSnapshot`/`ViewId` from `./layout.js`；`dispatchInput`/`submitWizardDraft`/`DispatchContext` from `./commands.js`；`wizardSubmit`/`WizardView` from `./wizard.js`；`parseCompanyYaml`、`EmployeeDefinition` from `@agenttown/runtime-contract`；`resolveAgentTownPaths` from `../paths.js`；`employeeStatus`、`record`、`requiredString`、`requiredNonnegativeInteger`、`friendlyError`、`CliClient` from `../main.js`
- Produces:
...
Modify `packages/cli/src/main.ts`：给 `record`（295 行）、`requiredString`（302 行）、`requiredNonnegativeInteger`（316 行）、`employeeStatus`（328 行）、`friendlyError`（431 行）加 `export` 关键字（仅关键字，无行为变化）。
